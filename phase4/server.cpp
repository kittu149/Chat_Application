...
void handle_client(int client_sock, struct sockaddr_in client_addr) {
    ...
    // 3. PoP RSA Signature
    std::string sig_hex;
    if (!rsa_sign_data(nonce + server_dh_pub, g_server_key_file, sig_hex)) {
        close(client_sock);
        return;
    }

    // 4. Send Certificate, Signature, and DH Key
    ...
    // 6. Establish Transport Session Key
    ...
    std::cout << "[PKI & CRYPTO] Client authenticated and link encrypted." << std::endl;
    ...
    // 8. Relay Loop (Forwards opaque tags __E2E_INIT__, __E2E_ACK__, __E2E_MSG__ blindly)
    while (true) {
        std::string msg;
        ...
        if (msg == "/who") {
            ...
            if (it != user_sessions.end()) {
                // Notice: For E2E messages, msg_content is opaque ciphertext __E2E_MSG__...
                // The server cannot read the inner conversation!
                std::cout << "[RELAY] (" << username << " -> " << recipient << "): " << msg_content << std::endl;
            ...
            send_encrypted_packet(client_sock, "ERROR: Invalid message format.", session_key);
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (g_server_cert_pem.empty()) {
        std::cerr << "[FATAL] Could not read certificate: " << cert_file << std::endl;
        return 1;
    }
    ...
    std::cout << "==================================================" << std::endl;
    std::cout << "   CS6008 Phase 4 Blind Relay Server" << std::endl;
