...
// ----------------- Phase 3 PKI & Proof-of-Possession -----------------
...
bool validate_certificate(const std::string& cert_pem, const std::string& ca_file, const std::string& expected_cn, std::string& out_error) {
    ...
    if (!bio) {
        out_error = "Could not create memory buffer for certificate.";
        return false;
    }
    ...
    BIO_free(bio);
    if (!cert) {
        out_error = "Failed to parse PEM X.509 certificate.";
        return false;
    }

    // 1. Verify CA signature and validity period using X509_STORE
    X509_STORE* store = X509_STORE_new();
    if (!store || X509_STORE_load_locations(store, ca_file.c_str(), NULL) != 1) {
        X509_free(cert);
        if (store) X509_STORE_free(store);
        out_error = "Failed to load trusted Root CA file: " + ca_file;
        return false;
    }
    ...
    if (!ctx || X509_STORE_CTX_init(ctx, store, cert, NULL) != 1) {
        ...
        out_error = "Failed to initialize X.509 verification context.";
        return false;
    }

    if (X509_verify_cert(ctx) != 1) {
        int err = X509_STORE_CTX_get_error(ctx);
        out_error = std::string("Certificate verification failed: ") + X509_verify_cert_error_string(err);
    ...
    // 2. Validate Common Name (CN)
    X509_NAME* subj = X509_get_subject_name(cert);
    char cn_buf[256] = {0};
    int cn_len = X509_NAME_get_text_by_NID(subj, NID_commonName, cn_buf, sizeof(cn_buf) - 1);
    if (cn_len < 0) {
        X509_free(cert);
        out_error = "Certificate has no Common Name (CN).";
        return false;
    }

    std::string cert_cn(cn_buf);
    if (!expected_cn.empty() && cert_cn != expected_cn) {
        out_error = "CN mismatch! Expected '" + expected_cn + "', but certificate is for '" + cert_cn + "'";
        X509_free(cert);
        return false;
    }
